...
DATA_PATH = os.path.join(ROOT_DIR, 'data','s3dis', 'Stanford3dDataset_v1.2_Aligned_Version')    ##拼接路径
g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/class_names.txt'))]  ##获取类别的列表[ ceiling floor wall beam column window door table chair sofa bookcase  board clutter]
g_class2label = {cls: i for i,cls in enumerate(g_classes)}   #   类别与对应的值 的字典    如 ceiling：0
g_class2color = {'ceiling':	[0,255,0],                                     #类型与颜色一一对应的字典
                 'floor':	[0,0,255],
                 'wall':	[0,255,255],
                 'beam':        [255,255,0],
                 'column':      [255,0,255],
...
g_label2color = {g_classes.index(cls): g_class2color[cls] for cls in g_classes}    #标签与颜色对应的字典


# -----------------------------------------------------------------------------
# CONVERT ORIGINAL DATA TO OUR DATA_LABEL FILES
#将原始数据转换为我们的datalabel文件
# -----------------------------------------------------------------------------

#将每个房间内的所有点加上标签 xyzrgbL
def collect_point_label(anno_path, out_filename, file_format='txt'):
    """ Convert original dataset files to data_label file (each line is XYZRGBL).
        We aggregated all the points from each instance in the room.
将原始数据集文件转换为data_label文件（每行为XYZRGBL）。 我们汇总了房间里每个实例的所有点。
    Args:
        anno_path: path to annotations. e.g. Area_1/office_2/Annotations/
        out_filename: path to save collected points and labels (each line is XYZRGBL)
        file_format: txt or numpy, determines what file format to save.   输出txt或者 numpy,
    Returns:
        None
    Note:
        the points are shifted before save, the most negative point is now at origin.保存之前会移动点，最负的点现在位于原点。
    """
    points_list = []
...
# V 每行以"v"开头，后面是x、y、z坐标以及RGB颜色值。
def data_to_obj(data,name='example.obj',no_wall=True):
    fout = open(name, 'w')
    label = data[:, -1].astype(int)    #数据的最后一行作为标签
    for i in range(data.shape[0]):
        if no_wall and ((label[i] == 2) or (label[i]==0)):     #去墙壁 0 天花板 2 墙壁
            continue
        fout.write('v %f %f %f %d %d %d\n' % \
                   (data[i, 0], data[i, 1], data[i, 2], data[i, 3], data[i, 4], data[i, 5]))    # V 每行以"v"开头，后面是x、y、z坐标以及RGB颜色值。
    fout.close()

#为了从data_ label文件可视化房间，根据标签可视化模型
...
def sample_data(data, num_sample):
    """ data is in N x ...
        ...
        数据单位为N x 。。。
        我们希望保留它们的num_sample x C
        如果N＞num_sample，我们将随机保留它们的num_sample
        如果N＜num_sample，我们将随机复制样本
    """
    N = data.shape[0]   #将data的行数赋值给N
    if (N == num_sample):
        return data, range(N) # range(N) 是一个用于生成整数序列的函数 0一直到N-1
    elif (N > num_sample):
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    准备块训练数据
    Args:
        data: N x 6 numpy array, 012 are XYZ in meters, 345 are RGB in [0,1]   xyzrgb
            assumes the data is shifted (min point is origin) and aligned
            (aligned with XYZ axis)  假设数据发生偏移（最小点为原点）并对齐（与XYZ轴对齐）
        label: N size uint8 numpy array from 0-12  标签
        num_point: int, how many points to sample in each block    每个块中采样的点数
        block_size: float, physical size of the block in meters     块的尺寸以米为单位
        stride: float, stride for block sweeping      步长
        random_sample: bool, if True, we will randomly sample blocks in the room   房间内随机采样
        sample_num: int, if random sample, how many blocks to sample  如果是随机采样，要采样多少块
            [default: room area]
        sample_aug: if random sample, how much aug   如果随机抽样，有多少aug
    Returns:
        block_datas: K x num_point x 6 np array of XYZRGB, RGB is in [0,1]   块数量x块内点数量x 6   xyzrgb
        block_labels: K x num_point x 1 np array of uint8 labels    块数量x块内点数量x 1  标签
...
def room2samples(data, label, sample_num_point):
    ...
    Returns:
        sample_datas: K x sample_num_point x 9
                     numpy array of XYZRGBX'Y'Z', RGB is in [0,1]   789为标准化三通道
...
def collect_bounding_box(anno_path, out_filename):
    """ Compute bounding boxes from each instance in original dataset files on
        ...
        从一个房间的原始数据集文件中的每个实例计算边界框**我们假设长方体与XYZ坐标对齐**
    
    Args:
        anno_path: path to annotations. e.g. Area_1/office_2/Annotations/
        out_filename: path to save instance bounding boxes for that room.保存该房间的实例边界框的路径。
            each line is x1 y1 z1 x2 y2 z2 label,
            where (x1,y1,z1) is the point on the diagonal closer to origin每条线是x1 y1 z1 x2 y2 z2标签，其中（x1，y1，z1）是对角线上更靠近原点的点
    Returns:
        None
    Note:
